...
load(file="prepMoth_April24_allSP_05km.RDATA")
...
MothCoords <- read.csv("Moths_109sites.csv", header=T)
head(MothCoords)


MothCoords$SiteID = as.numeric(MothCoords$SiteID)
MothCoords2 = MothCoords[MothCoords$SiteID %in% unique(Env.br2$SiteID), ]
coord = MothCoords2[order(match(MothCoords2$SiteID, unique(Env.br2$SiteID))),]
...
m1 = Hmsc(Y=Ypa, XData = XData,  XFormula = XFormula,
          ...
          ranLevels={list("site"=rL.spat, "year" = rL.year, "bg" = rL.biogeo)})

models = list(m1)
modelnames = c("presence_absence")

#save(models,modelnames,file = paste0("unfitted_models", scale, ".Rdata"))
...
filename_out = paste("MothTestApril", ch, "_models_thin_", as.character(thin),
