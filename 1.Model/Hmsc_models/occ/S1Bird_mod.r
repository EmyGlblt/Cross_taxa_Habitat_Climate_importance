...
load(file="prepBird_April24_Strat2_1km.RDATA")
...
bird_species = colnames(Ypa)

bird_species_df <- tax_name(bird_species, get = 'family', db = 'ncbi')
head(bird_species_df)

table(bird_species_df$family)
bird_species_df[is.na(bird_species_df$family),]
...
save(models,modelnames,file = paste0("unfitbird_models", ch, ".Rdata"))
...
filename_out = paste("BirdtestAp", ch, "_models_thin_", as.character(thin),
