...
load(file="prepButterflyTransects_Sept23_allSP_05km.RDATA")
...
#array job at CSC with 4 parallel runs = one for each of 4 chs
...
bfCoords <- read.csv("ButterflyTransects_centroids.csv", header=T)
head(bfCoords)

bfCoords$Transect = as.numeric(bfCoords$Transect)
coord = bfCoords[order(bfCoords$Transect),]
...
save(models,modelnames,file = paste0("unfitbfab_models", ch, ".Rdata"))
...
filename_out = paste("bfAB", ch, "_models_thin_", as.character(thin),
