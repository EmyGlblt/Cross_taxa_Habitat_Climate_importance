...
pal = c('#66C2A5', 'yellow3', 'orange3')


# prepare the plotting
rc_un = data.frame(col_name= c("Climate", "LandscapeConf", "HabitatComp"),
                   row_name=c("Climate", "LandscapeConf", "HabitatComp"))
rc_un$row_name = factor(rc_un$row_name, 
                        levels = c("Climate", "LandscapeConf","HabitatComp"))
rc_un$col_name = factor(rc_un$col_name, 
                        levels = c("Climate", "LandscapeConf", "HabitatComp"))


VP_C_all_occ.sub = VP_C_all_occ[VP_C_all_occ$col_name %in% c("Climate", "LandscapeConf","Habitatcomp") &
                                  VP_C_all_occ$row_name %in% c("Climate", "LandscapeConf","Habitatcomp"),]
...
VP_C_all_ab.sub = VP_C_all_ab[VP_C_all_ab$col_name %in% c("Climate", "LandscapeConf","Habitatcomp") &
                                  VP_C_all_ab$row_name %in% c("Climate", "LandscapeConf","Habitatcomp"),]
...
gg_old = {ggplot() +
  ...
  modify_facet_appearance(strip.background.x.fill = pal,
                          strip.background.x.col = rep('black', 3),
                          strip.text.x.col = rep('black', 3),
                          strip.background.y.fill = pal,
                          strip.background.y.col =rep('black', 3),
                          strip.text.y.col = rep('black', 3))
...
# Higher quality
# library(pdftools)
# bitmap <- pdf_render_page("Figure2.pdf", page = 1, dpi = 600)
# png::writePNG(bitmap, "Figure2_hr.png")
...
gp_ab = {ggplot(data = VP_C_mean_ab, aes(y=Propmean, x=value, fill=taxa)) +
    ...
    facet_grid(row_name ~ col_name, drop = FALSE, switch = "y") + 
    coord_curvedpolar(start = -pi/12) +
    geom_hline(yintercept = seq(0, 0.74, by = 0.25), colour = "darkgrey", size = 0.55) +
    geom_hline(yintercept = 0.25, colour = "gray40", size = 0.75) +
    geom_hline(yintercept = 0.75, colour = "gray40", size = 0.75) +
    geom_vline(xintercept = c(0.5, 1.5, 2.5, 3.5), colour = "gray40", size = 0.85) +
    scale_y_continuous(breaks = seq(0, 0.75, by = 0.25)) +
